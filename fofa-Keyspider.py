...
def http(url):
    try:
        r = requests.get(url=url,verify=False,proxies={ "http": "http://127.0.0.1:8080", "https": "http://127.0.0.1:8080"})#proxies={ "http": "http://127.0.0.1:8080", "https": "http://127.0.0.1:8080"}
        return r.text
...
def ip(ip):
    ...
    res = http('https://fofa.so/api/v1/search/all?email=%s&key=%s&qbase64=%s&size=10000&page=1&fields=ip,host,title,port,protocol'%(define.FOFA_EMAIL,define.Apikey,str(qbase,'utf-8')))
    [write_xlsx(i) for i in json.loads(res)["results"]]

def keyword(key):
    #qbase = base64.b64encode(str("%s"%key).encode('utf-8'))
    #for page in range(1,100):
    res = http('https://fofa.so/api/v1/search/all?email=%s&key=%s&qbase64=%s&size=10000&page=1&fields=ip,host,title,port,protocol'%(define.FOFA_EMAIL,define.Apikey,key))
    ...
    for i in json.loads(res)["results"]:
        ...
        sheet1 = wb['Sheet']
        num = sheet1.max_row
        sheet1.cell(row = num+1,column = 1,value = i[0])
        sheet1.cell(row = num+1,column = 2,value = i[1])
        sheet1.cell(row = num+1,column = 3,value = i[2])
        sheet1.cell(row = num+1,column = 4,value = i[3])
        sheet1.cell(row = num+1,column = 5,value = i[4])
